<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>결과 - 하쿠</title>
    <link rel="stylesheet" href="css/result5.css" />
    <link rel="stylesheet" href="css/font.css" />
</head>

<body>
    <section class="top-section">

        <div class="main-title">
            <span id="title">하쿠</span>
        </div>


        <div class="first">
            <span id="semi-title">신비로운 강의 신</span>
        </div>

        <div id="greenLine">
            <img src="img/greenLine.png" alt="초록선" class="greenLine">
        </div>

        <section class="haku-section">
            <img src="img/result_haku.png" alt="하쿠 결과" class="haku_img">
            <span class="main-info" id="info">
                침착하고 신비로운 지혜를 품은 수호자, 하쿠. <br>
                겉으로는 냉정하고 차분해 보이지만, <br>
                그 안에는 따뜻함과 책임감이 가득합니다. <br>
                수수께끼 같은 분위기 속에서 자신의 역할을 수행하며, <br>
                위기에 처한 치히로에게는 든든한 조력자가 됩니다. <br>
                자신보다 타인을 먼저 생각하는 태도는 <br>
                하쿠처럼 깊은 마음을 보여줍니다. <br>
                <br>
                기억을 잃은 채 살아가면서도 <br>
                마음속에 남은 진심으로 사람을 돕는 모습은, <br>
                우리가 잊고 살던 본래의 모습을 떠오르게 합니다. <br>
                당신도 하쿠처럼 누군가에게 조용히 힘이 되어주는 <br>
                존재일 수 있어요.   
            </span>
        </section>

        <div class="second">
            <span id="semi-title">성격 포인트</span>
        </div>

        <div class="point-info">
            <span id="info">
                • 침착하고 이성적인 판단력을 지니고 있어요. <br>
                • 감정보다 책임감을 우선시하는 스타일이시네요. <br>
                • 신비롭고 차분한 분위기로 사람들을 끌어당겨요. <br>
                • 마음속에는 깊은 애정과 희생정신이 자리하고 있어요.
            </span>
        </div>

        <div id="haku_icon">
            <img src="img/haku_icon.png" alt="하쿠 아이콘" class="haku_icon">
        </div>

        <div class="third">
            <span id="semi-title">당신이 가진 하쿠 같은 매력</span>
        </div>

        <div class="rizz-info">
            <span id="info">
                • 무심한 듯 보이지만 가장 먼저 보여주는 사람 <br>
                • 조용하지만, 누구보다보다 신뢰할 수 있는 존재 <br>
                • 마음의 상처가 있어도, 그 속에서 따뜻함을 잃지 않는 사람
            </span>
        </div>

    </section>

    <section class="match-section">
        <div class="match-title">
            <span id="main-title">”하쿠”와 잘 맞는 캐릭터</span>
        </div>
        <a href="result6.html" tabindex="0">
            <img src="img/to_chihiro.png" alt="치히로 아이콘" class="match-img no-drag">
        </a>
        <div class="match-name">
            <span id="match-name">치히로</span>
        </div>
        <div class="match-info">
            <span id="info">용기있는 법을 알려주는 당신을 바라보며 점점 마음이 단단해지는 사람이에요.</span>
        </div>
    </section>

    <section class="footer">
        <img src="img/result-footer.png" alt="결과 푸터" class="result-footer no-drag" />
        <a href="guestbook4.php" tabindex="0" id="footer-info-link">
            <span id="footer-info">방명록으로 가기</span>
        </a>
        <a href="guestbook4.php" tabindex="0" class="result-arrow-link">
            <img src="img/result-arrow.png" alt="결과 버튼" class="result-arrow no-drag" />
        </a>
    </section>

</body>

</html>